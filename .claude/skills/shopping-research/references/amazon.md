...
`reviews()` gives you `sampling: {n, ratingsTotal, coverage, ceiling}`. On one ring: 8 readable
against 574 rated is **1.4% coverage**, while the distribution says 3% are 1★ — roughly 17 angry
reviews the "one star" filter will not show you. Quote the distribution and the coverage figure.
Never characterise a product's problems from the sample.
